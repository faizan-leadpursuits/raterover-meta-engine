...
class BookingComAdapter(HotelBaseAdapter):
    NAME = "booking_com"
    DOMAIN = "hotels"
    NEEDS_PROXY = True
    ...
    def search(self, city, check_in, check_out, adults=2, children=0, rooms=1, currency="USD", **kw):
        from .scraper import BookingComScraper

        print(f"  [{self.NAME}] Searching hotels in {city}...")
        filters = kw.get("filters", None)
        extra = dict(kw)
        extra.pop("filters", None)
        raw = BookingComScraper().search(
            city,
            check_in,
            check_out,
            adults,
