"""
Destinia hotel scraper — curl_cffi HTML scraper for www.destinia.com.
"""
...
class DestiniaScraper:
    ...
    def search(self, city: str, check_in: str, check_out: str,
        ...
        logger.info("[destinia] Searching '%s'...", city)
        url = "https://www.destinia.com/hotels/en/search/{city}?checkin={check_in}&checkout={check_out}&adults={adults}&rooms={rooms}".format(
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning("[destinia] Status %d", resp.status_code)
                return []
            return self._parse(resp.text or "", city, currency)
        except Exception as e:
            logger.error("[destinia] Error: %s", e)
            return []
