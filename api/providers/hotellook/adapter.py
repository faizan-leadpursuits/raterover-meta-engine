"""Hotellook adapter — normalizes Hotellook API output to universal hotel schema."""
...
from .scraper import HotellookScraper

logger = logging.getLogger(__name__)


class HotellookAdapter(HotelBaseAdapter):
    NAME = "hotellook"
    DOMAIN = "hotels"
    NEEDS_PROXY = False
    ...
    def search(self, city: str, check_in: str, check_out: str,
        ...
        try:
            scraper = HotellookScraper(proxy=proxy)
            raw = scraper.search(city, check_in, check_out, adults, rooms, currency)
        except Exception as e:
            logger.error("[%s] Scraper error: %s", self.NAME, e)
            return self.empty_result()
        ...
        for h in raw:
            try:
                ...
                rows.append(make_hotel_row(
                    ...
                    booking_provider="Hotellook",
                    cancellation="", amenities="",
                    deep_link=h.get("deep_link", ""),
                    image_url=h.get("image_url", ""),
                ))
            except Exception as e:
                logger.warning("[%s] Normalize error: %s", self.NAME, e)
