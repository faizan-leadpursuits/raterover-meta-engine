"""
Hotwire hotel scraper — curl_cffi HTML scraper for www.hotwire.com.
"""
...
class HotwireScraper:
    ...
    def search(self, city: str, check_in: str, check_out: str,
        ...
        logger.info("[hotwire] Searching '%s'...", city)
        url = "https://www.hotwire.com/hotels/search?destination={city}&startDate={check_in}&endDate={check_out}&adults={adults}&rooms={rooms}".format(
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning("[hotwire] Status %d", resp.status_code)
                return []
            return self._parse(resp.text or "", city, currency)
        except Exception as e:
            logger.error("[hotwire] Error: %s", e)
            return []
