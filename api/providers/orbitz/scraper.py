"""
Orbitz hotel scraper — curl_cffi HTML scraper for www.orbitz.com.
"""
...
class OrbitzScraper:
    ...
    def search(self, city: str, check_in: str, check_out: str,
        ...
        logger.info("[orbitz] Searching '%s'...", city)
        url = "https://www.orbitz.com/Hotel-Search?destination={city}&startDate={check_in}&endDate={check_out}&adults={adults}&rooms={rooms}".format(
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning("[orbitz] Status %d", resp.status_code)
                return []
            return self._parse(resp.text or "", city, currency)
        except Exception as e:
            logger.error("[orbitz] Error: %s", e)
            return []
